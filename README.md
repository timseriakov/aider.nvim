...
Some features are lacking, but it's also more light weight. For example, the new plugin doesn't watch all writes for Aider comments, instead it assumes you're using the plugins function to add the comment so it can ensure Aider is running before adding it to the file. Additionally the new plugin provides a function to add files in Aider, but integration with file pickers is left to the end user (although a working example for snacks integration is provided). New feature requests are welcome in the new repository.
